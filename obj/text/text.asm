...
00101$:
	...
	or	a, a
	jr	Z,00103$
;src/text/text.c:27: result++;
	inc	c
	jr	00101$
00103$:
;src/text/text.c:29: return result;
	ld	l,c
	ret
;src/text/text.c:35: void strCopy(i8* to, const i8* from) {
;	---------------------------------
; Function strCopy
; ---------------------------------
_strCopy::
...
00101$:
	...
	or	a, a
	jr	Z,00104$
;src/text/text.c:39: *to++ = *from++;
	ld	a,(bc)
	inc	bc
	ld	(de),a
	inc	de
;src/text/text.c:40: i--;
	dec	-1 (ix)
	jr	00101$
00104$:
	inc	sp
...
;src/text/text.c:52: itoa(aNumber, str, 10);
	...
	ld	sp,hl
	pop	bc
;src/text/text.c:54: zeros = length - strLength(str);
	ld	e, c
	ld	d, b
	push	bc
	push	de
...
;src/text/text.c:60: pvideo = cpct_getScreenPtr(CPCT_VMEM_START, (zeros + x) * FONT_W + xPos, yPos);
	...
	pop	de
	pop	bc
;src/text/text.c:61: cpct_drawSprite(G_numbers_big[number - 48], pvideo, FONT_W, FONT_H);
	push	hl
	pop	iy
	ld	d,#0x00
	ld	a,e
...
;src/text/text.c:75: if (centered) {
	...
	or	a, a
	jr	Z,00102$
;src/text/text.c:76: x = strLength(text);
	ld	l,4 (ix)
	ld	h,5 (ix)
	push	hl
	call	_strLength
...
;src/text/text.c:77: xPos = 39 - (x / 2) * FONT_W;
	...
	ld	6 (ix),a
00102$:
;src/text/text.c:81: character = text[x];
	ld	c,4 (ix)
	ld	b,5 (ix)
	ld	a,(bc)
	ld	-5 (ix),a
